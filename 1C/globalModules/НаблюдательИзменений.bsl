...
Процедура ЗаписатьВоВременныйJSONФайл(Данные) Экспорт
...
Функция ЗаполнитьСостояниеОбъекта(ИзмОб, СтруктураСостоянийОбъекта, Режим) Экспорт
	...
	ИначеЕсли Режим = 3 Тогда
		...
		Если РезультатСравнения.OLD.Количество() = 0
			ИЛИ РезультатСравнения.NEW.Количество() = 0 Тогда
...
Функция СравнитьСтруктуры(СтруктураДо, СтруктураПосле)
	...
	РезультатДо = Новый Структура;
	РезультатПосле = Новый Структура;
	...
	Для каждого текПоле Из СтруктураДо Цикл
		...
		Если (текПоле.Значение <> ЗначениеСравнения) ИЛИ (ЗначениеСравнения = Неопределено) Тогда
			...
			РезультатДо.Вставить(текПоле.Ключ, СтруктураДо[текПоле.Ключ]);
			РезультатПосле.Вставить(текПоле.Ключ, ЗначениеСравнения);	
		
		КонецЕсли; 
	
	КонецЦикла; 
	
	Результат.Вставить("OLD", РезультатДо);
	Результат.Вставить("NEW", РезультатПосле);
	Возврат Результат;
