&НаСервере
Функция ПолучитьНеобходимыеСведенияДляНаблюдателя(Значение)
	
	НеобходимаяИнфо = Новый Структура;
	
	ЧастиИмени = СтрРазделить(Значение.Метаданные().ПолноеИмя(), ".");
	НеобходимаяИнфо.Вставить("ЭтоВнешняяОбработка",(ВРег(ЧастиИмени[0]) = "ВНЕШНЯЯОБРАБОТКА"));	
	...
	Возврат НеобходимаяИнфо;
	
КонецФункции // ()


&НаСервере
Процедура ВестиНаблюдение(Значение) Экспорт
	Сведения = ПолучитьНеобходимыеСведенияДляНаблюдателя(Значение);
	// логика такова: 
	...
	Если НЕ Сведения.ИмяКомпьютера = "sws034" ИЛИ НЕ Сведения.ЭтоВнешняяОбработка Тогда
		Возврат;
	Иначе
